...
bool SOCKS5Address::parse(const string &data, size_t &address_len) {
  ...
  case DOMAINNAME: {
    ...
    if (data.length() > (unsigned int)(1 + domain_len + 2)) {
      ...
      port =
          (uint8_t(data[domain_len + 2]) << 8) | uint8_t(data[domain_len + 3]);
  ...
  case IPv6: {
    if (data.length() > 16 + 2) {
      ...
      sprintf(t,
              ...
              uint8_t(data[4]), uint8_t(data[5]), uint8_t(data[6]),
              uint8_t(data[7]), uint8_t(data[8]), uint8_t(data[9]),
              uint8_t(data[10]), uint8_t(data[11]), uint8_t(data[12]),
              uint8_t(data[13]), uint8_t(data[14]), uint8_t(data[15]),
              uint8_t(data[16]));
      address = t;
